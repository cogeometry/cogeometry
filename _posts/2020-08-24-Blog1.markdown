...
4. Adic spaces: Adic spaces, and this can already be seen as a first kind of complication, are not defined in terms of a ring $$A$$ with a topology, but in terms of pairs of rings $$(A,A^+)$$, where we can think of $$A^+\subset A$$, in some instances at least, as subring of bounded elements in $$A$$. The most canonical example is $$(K,\Ox_K)$$. The construction of the adic space $$Spa(A,A^+)$$ is similar to the one just mentioned and is in terms of valuations (or rather equivalence classes of such). But allowing more general valuations (to be specified later), one gets even more points. While this approach has been used heavily recently, see [ref berkeley], it has some rather horrid behaviour, e.g. when it comes to fibre products, as they might not even exist! And here we might end up in a circular loop, as one might go back to formal schemes, as they behave much better in terms of limits than adic spaces.